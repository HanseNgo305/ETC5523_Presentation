...
sensor_reading_location <- sensor_reading_location %>%
  ...
  select(-last_data,-location, -units) %>%
  mutate(time = strftime(local_time, format="%H:%M:%S", tz = "AEDT"),
         day = day(local_time),
...
monthly_pm_2.5 <- daily_pm_2.5 %>% ungroup() %>%
  group_by(site_id, year, month) %>%
